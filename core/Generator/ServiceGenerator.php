...
class ServiceGenerator extends Generator
{
     ...
     * 创建业务模块服务层.
     */
    public function createModuleService(): array
    {
        // 未设置 Module 暂不支持创建
        ...
        $class = 'Service';
        $this->withStub('init/service.stub');
        $this->withNamespace('App\\' . $this->moduleInfo->getName() . '\\Service');
     ...
     * 创建业务模块服务层.
     */
    public function createService(string $name, ?string $path = null): array
    {
        ...
        $uses = '';
        $class = ucfirst($name);
        $this->withStub('service.stub');
        $this->withNamespace('App\\' . $this->getModuleInfo()->getName() . '\\Service');
        if ($path) {
            $uses = $this->getNamespace() . '\\Service';
